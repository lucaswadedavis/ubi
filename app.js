...
function restart() {
  ...
  g.append('svg:circle')
    ...
    .on('mouseup', function(d) {
      ...
      // todo(luke): add the edge addition call here
...
function increment() {
  for (var i = 0; i < nodes.length; i++) {
    ...
    for (var j = 0; j < source.links.length; j++) {
      if (source.links.length === 0) continue;
      source.links[j].nextValue += source.value / source.links.length;
      source.value -= source.value / source.links.length;
    }
  }

  for (var i = 0; i < nodes.length; i++) {
    nodes[i].value += nodes[i].nextValue;
    nodes[i].nextValue = 0;
  }
